# @pinwheel/capacitor-sdk — Capacitor 6 Example App

React + Vite + Capacitor 6 sample that creates Pinwheel Link tokens and opens the native SDK via the local plugin (`file:..`).
...
**2. Install example-app-capacitor-6 dependencies** (from this directory):
...
Opens at `http://localhost:5176`. The native `Pinwheel.open()` call will no-op in the browser — useful for UI iteration only.
